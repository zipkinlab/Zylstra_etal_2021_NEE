...
#County-year-week covariates 
  X_county <- as.matrix(cyw[,c('feb','spGDD','spGDD2','spPCP','spPCP2'
                               ,'avgGDD','diffGDD','diffGDD2','diffavgGDD','suPCP'
                               ,'gly','crop','glycrop')]) 
...
#-Save----------------------------------------------------------------#
  ...
  date <- Sys.Date()
  file_str <- paste0('GitHubtest_FACM_2004-2018_HPwinter','_',date,'.Rdata')
    
  save(sessionInfo, out.null, out.S, out.N, out.C, out.SN, out.SC, out.NC, out.SNC, 
       summer, winter, cyw, hp.w, file=file_str)    
